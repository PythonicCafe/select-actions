...
stop:   ## Stop container
	docker stop select_actions_dev

watch:  ## Runs a lite-server with examples accessible in http://localhost:3000
	docker exec -it select_actions_dev sh -c "yarn run watch & yarn lite-server"

bundle: ## Make a bundle local instalable pakage '.tgz'
	docker exec  select_actions_dev sh -c "yarn run build; yarn pack"

eslint: ## Run eslint
	docker exec select_actions_dev sh -c "yarn run eslint"

prettier: ## Run prettier
	docker exec select_actions_dev sh -c "yarn run prettier"

interact: ## Get access to /bin/sh interactive mode
	docker exec -it select_actions_dev /bin/sh

test:	## Run tests scripts located in /tests
	docker exec select_actions_dev sh -c "yarn run build; yarn lite-server & yarn run test"
